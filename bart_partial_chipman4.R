...
bart4 <- bartMachine(X = train_covariates, y = train$Potency, num_trees = 1000, num_burn_in = 20000, num_iterations_after_burn_in = 20000,
                     alpha = 0.95, k = 2, nu = 3, beta = 2, serialize = T)

test_covariates <- as.data.frame(select(test, -Potency))
bart4_test <- bart_predict_for_test_data(bart4, test_covariates, test$Potency, prob_rule_class = NULL)

save(bart4, file = paste0(path, "/bart4_spam.RData"))
save(bart4_test, file = paste0(path, "/bart4_spam_test.RData"))
