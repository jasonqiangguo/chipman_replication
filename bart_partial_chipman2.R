...
bart2 <- bartMachine(X = train_covariates, y = train$Potency, num_trees = 1000, num_burn_in = 20000, num_iterations_after_burn_in = 20000,
                     alpha = 0.95, k = 2, nu = 3, beta = 2, serialize = T)

test_covariates <- as.data.frame(select(test, -Potency))
bart2_test <- bart_predict_for_test_data(bart2, test_covariates, test$Potency, prob_rule_class = NULL)

save(bart2, file = paste0(path, "/bart2_spam.RData"))
save(bart2_test, file = paste0(path, "/bart2_spam_test.RData"))
